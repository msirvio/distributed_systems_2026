...
from datetime import datetime, time #import datetime for storing last update time for patients, and also for printing it in a nice format
...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///hospital_b.db" #Tells flask to which database to connect to, and store database in file called "test.db"
...
api = Api(app)
HOSPITAL_ID = os.getenv("HOSPITAL_ID", "hospital_b")
...
if __name__ == "__main__":
    ...
    app.run(debug=True, port=5001) #Runs the Flask application in debug mode, which provides helpful error messages and auto-reloads the server when code changes are detected
